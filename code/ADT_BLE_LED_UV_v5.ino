...
// To-Do:
//    * Add fields to and prefs to set gmt offset, daylight savings, ntp server.
//    * Display current settings next to new settings fields
...
const long gmtOffset_sec = -28800; // Adjust for your timezone (e.g., -8 hours for PST)
...
void setup()
{
  ...
  else Serial.println("SPIFFS Initialized.");
  ...
  prefs.begin("faders", false);
    DayBrightness[STRIP_R] = prefs.getUInt("stripRday", 230);    
    DayBrightness[STRIP_G] = prefs.getUInt("stripGday", 100);    
    DayBrightness[STRIP_B] = prefs.getUInt("stripBday", 255);    
    ...
    FadeSeconds[AMBI_R] = prefs.getUInt("ambiRfade", 1800);    
...
/*
    // Endpoint to start OTA:
    server.on("/start_ota", HTTP_GET, [](AsyncWebServerRequest *request){
        server.reset(); // Clears all handlers, effectively stopping the server from accepting new requests
        ArduinoOTA.begin(); // Start OTA only when requested
        request->send(200, "text/plain", "OTA started. Webserver stopped.");
        otaActive = true;
    });
*/
...
/*      jsonDoc["globalBrightness"] = globalBrightness;
      ...
      jsonDoc["DayBrightness[STRIP_R]"] = DayBrightness[STRIP_R];
      jsonDoc["DayBrightness[STRIP_G]"] = DayBrightness[STRIP_G];
      jsonDoc["DayBrightness[STRIP_B]"] = DayBrightness[STRIP_B];
      jsonDoc["DayBrightness[AMBI_R]"] = DayBrightness[AMBI_R];
      jsonDoc["DayBrightness[AMBI_UV]"] = DayBrightness[AMBI_UV];
/*      jsonDoc["Pattern"] = pattern;
    ...
    server.on("/setNetTime", HTTP_GET, [](AsyncWebServerRequest *request){
...
void loop()
{
  ...
  delay(10);   // don't hog the CPU, give some time to AWS
...
void ReCalcADT(void) {
  ...
  yearFraction += 171.0;    // Offset calendar so that day 0 is Jun 21 (Spring Equinox)
  if(yearFraction > 365.0) yearFraction -= 365.0;
...
void netTimetoRTC(void) {
  ...
  MCP7940.adjust(DateTime(timeinfo.tm_year, timeinfo.tm_mon, timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec));
